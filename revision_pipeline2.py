...
def normalize(image):
    ...
    if max_val - min_val < 1e-8:  # 완전히 빈 슬라이스만 제거
        return None
...
plt.title("MRI 복원 Loss 곡선 (v3)")
plt.xlabel("Epoch")
plt.ylabel("MSE Loss")
plt.grid(True)
plt.tight_layout()
plt.savefig("mri_loss_v3.png", dpi=150)
plt.show()
...
plt.suptitle("MRI 복원 결과 v3", fontsize=13, fontweight='bold')
plt.tight_layout()
plt.savefig("mri_result_v3.png", dpi=150)
plt.show()

torch.save(model.state_dict(), "unet_mri_v3.pth")
print("완료!")